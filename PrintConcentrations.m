...
% änderungen tobi, am 5.10.18:
%pause auskommentiert, fullscreen rein, beide kohlenstoff beispiele im plot,
%data_kpp umbenannt im atcsol abschnitt
%zuordnung der Spezies/nummern für ausgabe hinzugefügt
%% 
close all;
clear all;
...
DATA_AtCSol = textscan(fileID,'%s', 'Delimiter', 't', 'HeaderLines',0);

 
DATA_AtCSol = DATA_AtCSol{1,1};
...
for i=1:n_Species
  disp([num2str(i),'    ',char(Names_KPP{1,i})])
  for j=1:n_Species
...
% for i = [5,8] %size(Names_KPP,2)-1
for i = [5,8,23,22,26] %size(Names_KPP,2)-1
    
    %5:c1ccccc1
    %8:c1ccccc1C
    figure('units','normalized','outerposition',[0 0 1 1]);% hier das i entfernt
    %    figure;
